...
import { workExperiences, travels, speakingEngagements, crafts } from "@workspace/content";
import { MapPin, Calendar, ArrowRight, ArrowUpRight, BookOpen, ExternalLink, Linkedin, Instagram } from "lucide-react";
...
export default function Home() {
  ...
  const onSubmit = async (data: z.infer<typeof contactSchema>) => {
    if (!FORMSPREE_FORM_ID) {
      toast({ variant: "destructive", title: "Error", description: "Contact form is not configured yet." });
      return;
    }
    ...
    <div className="min-h-screen">
      ...
      <section id="travels" className="py-32 px-6 bg-primary text-primary-foreground">
        <div className="container mx-auto">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end mb-16 gap-8">
            ...
            <Link
              href="/travels"
              className="text-sm uppercase tracking-widest text-primary-foreground/60 hover:text-primary-foreground transition-colors flex items-center gap-2 group"
            >
              {travels.length}+ destinations
          ...
          {travels.length > 8 && (
            <div className="mt-8 text-center text-primary-foreground/60 text-sm">
              + {travels.length - 8} more destinations
            </div>
          )}
        </div>
      </section>
      ...
      <section id="speaking" className="py-32 px-6 bg-background">
        <div className="container mx-auto grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
          <Link href="/speaking" className="relative aspect-[4/3] rounded-3xl overflow-hidden shadow-2xl group block">
            ...
            <img src="/images/speaking-google.jpg" alt="Sarah speaking on stage at Google" className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
            <div className="absolute bottom-5 right-5 z-20 flex items-center gap-2 bg-white/90 text-foreground text-xs font-bold uppercase tracking-wider px-4 py-2 rounded-full opacity-0 group-hover:opacity-100 transition-all duration-300 translate-y-2 group-hover:translate-y-0">
              View Speaking <ArrowUpRight className="w-3.5 h-3.5" />
            </div>
          </Link>
